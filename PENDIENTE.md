...
| **Retrato del profesor** | `PROFESOR.foto` en `sitio.ts` | El nombre ya está: **Chris Pow**, confirmado por el cliente el 22/09/2026. Falta la foto, y hasta que llegue `/online` pinta el marco con su aviso. Va **retrato**, porque el hueco está cortado a 4:5; una apaisada se recorta por los lados. Ponerla es una línea: la ruta dentro de `public/` |
| **Biografía del profesor** | `paginas.online.profesorTexto` en los dos diccionarios | Hoy dice lo genérico —costarricense en Suiza, enseña el español de allá—. Con un par de frases suyas de verdad, esa sección deja de ser una plantilla |
